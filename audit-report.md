...
The structural and site-quality checks pass. The audit found two focused defects, a low-contrast widget-count label on the home-page cards and an absent `robots.txt` file behind Vercel's SPA rewrite, and fixes both in this change. It also records the outstanding prose-style finding below rather than folding a broad chapter rewrite into this audit item.
...
| Widgets using `WidgetCard` and receiving `tryThis` | 123 / 123 |
| Static diagrams with `role="img"` and `aria-label` | 97 / 97 |
| Source range controls | 85, named at runtime by `WidgetAccessibility` when a local label is absent |
...
| Em dashes in chapter prose | 1,392 lines, outstanding style-guide finding |
| Banned stock phrasing in chapter prose | 1 occurrence, outstanding style-guide finding |

The route, page, widget, diagram, citation, cross-reference, and placeholder rows use `rg`/file-inventory checks. The range-control result is based on the source inventory and the shared runtime labeling path in `src/components/shared/WidgetAccessibility.jsx`. The final two rows are deliberate audit findings: the V2 style guide bans em dashes and stock phrasing, but correcting those 1,393 instances requires a broad editorial pass and is not folded into this focused Q5 change.
...
The measured web-vitals diagnostics were FCP 2.0 s, LCP 2.0 s, TBT 0 ms, CLS 0, Speed Index 2.0 s, and Time to Interactive 2.0 s. Lighthouse estimates 600 ms of render-blocking CSS opportunity and 41 KiB of unused initial JavaScript. These are baseline observations, not claims about a deployed CDN response or field performance.
...
`context/CURRICULUM.md` and `context/PROJECT_OVERVIEW.md` now describe the completed 25-chapter V2 migration. `context/PROJECT_OVERVIEW.md` also distinguishes `npm run build` from the full `npm run check` gate. `context/STYLE_GUIDE.md` no longer points to the already-completed Q3 mobile work as future refinement.
