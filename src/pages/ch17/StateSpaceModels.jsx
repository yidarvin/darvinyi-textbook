...
export default function StateSpaceModels() {
  ...
  return <article style={{ maxWidth: 'var(--chapter-max-width, 740px)', margin: '0 auto', padding: 'var(--chapter-padding, 52px 44px 100px)' }}>
    ...
    <MathBlock>{'$$x_t = \\bar A x_{t-1} + \\bar B u_t, \\qquad y_t = Cx_t + Du_t$$'}</MathBlock>
    <p style={prose}>Here <InlineMath>{'u_t'}</InlineMath> is the input at position <InlineMath>{'t'}</InlineMath>, <InlineMath>{'x_t'}</InlineMath> is the persistent state, <InlineMath>{'\\bar A'}</InlineMath> and <InlineMath>{'\\bar B'}</InlineMath> are the discretized transition and input maps, and <InlineMath>{'C,D'}</InlineMath> read the state and current input into <InlineMath>{'y_t'}</InlineMath>. This is a linear time-invariant system when the matrices stay fixed across positions.</p>
    <p style={prose}>The same fixed SSM has two exact views. It can run recurrently, updating one state at a time, or it can form a causal convolution kernel and apply that kernel to the whole input. For a scalar worked example with <InlineMath>{'a = 0.75'}</InlineMath> and inputs <InlineMath>{'[1, 0, 0]'}</InlineMath>, the recurrence gives outputs <InlineMath>{'[1, 0.75, 0.5625]'}</InlineMath>; convolving with <InlineMath>{'[1, 0.75, 0.5625]'}</InlineMath> gives the same three numbers.</p>
    ...
    <MathBlock>{'$$\text{hybrid KV state per generated token} \propto L_{\mathrm{attn}}\,d,\qquad \text{scan state} \propto L_{\mathrm{scan}}\,d$$'}</MathBlock>
    <p style={prose}>Here <InlineMath>{'L_{\mathrm{attn}}'}</InlineMath> and <InlineMath>{'L_{\mathrm{scan}}'}</InlineMath> count attention and scan layers. Each attention layer adds a key and value for every earlier token; scan layers instead carry a fixed state per layer.</p>
    ...
    <ExerciseCard number="01">For the scalar SSM <InlineMath>{'x_t = 0.8x_{t-1} + u_t'}</InlineMath> with <InlineMath>{'x_0=0'}</InlineMath> and inputs <InlineMath>{'[1,0,0]'}</InlineMath>, compute the three states. Then write its first three convolution-kernel values and verify the same output.</ExerciseCard>
