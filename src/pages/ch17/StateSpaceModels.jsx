...
export default function StateSpaceModels() {
  ...
  return <article style={{ maxWidth: 'var(--chapter-max-width, 740px)', margin: '0 auto', padding: 'var(--chapter-padding, 52px 44px 100px)' }}>
    ...
    <p style={prose}>Here <InlineMath>{'u_t'}</InlineMath> is the token input at position <InlineMath>{'t'}</InlineMath>, <InlineMath>{'x_t'}</InlineMath> is the sampled persistent state, and the barred maps are the transition and input maps produced by the step size <InlineMath>{'\\Delta'}</InlineMath>. When they stay fixed across positions, unrolling the recurrence with <InlineMath>{'x_0=0'}</InlineMath> gives <InlineMath>{'y_t=\\sum_{\\ell=0}^{t}K_{\\ell}u_{t-\\ell}+Du_t'}</InlineMath>, where <InlineMath>{'K_{\\ell}=C\\bar A^{\\ell}\\bar B'}</InlineMath>. That is the causal convolution kernel.</p>
    <p style={prose}>The same fixed SSM therefore has two exact views: update one state at a time, or apply its kernel to the whole input. The widget uses the scalar case <InlineMath>{'\\bar A=a'}</InlineMath>, <InlineMath>{'\\bar B=C=1'}</InlineMath>, and <InlineMath>{'D=0'}</InlineMath>, so <InlineMath>{'K_{\\ell}=a^{\\ell}'}</InlineMath>. With <InlineMath>{'a = 0.75'}</InlineMath> and inputs <InlineMath>{'[1, 0, 0]'}</InlineMath>, both computations give <InlineMath>{'[1, 0.75, 0.5625]'}</InlineMath>.</p>
