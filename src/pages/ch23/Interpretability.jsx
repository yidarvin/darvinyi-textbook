...
const CITATIONS = buildCitations([
  ...
  { title: 'Circuit Tracing: Revealing Computational Graphs in Language Models', authors: 'Ameisen, Lindsey, Pearce, Gurnee, Turner, Chen, Citro, et al.', venue: 'Transformer Circuits Thread', year: '2025', tag: 'paper' },
]);
...
export default function Interpretability() {
  ...
  return <article style={{ maxWidth: 'var(--chapter-max-width, 740px)', margin: '0 auto', padding: 'var(--chapter-padding, 52px 44px 100px)' }}>
    ...
    <p style={prose}>Start at two features, then increase the feature count while keeping the representation two-dimensional. Notice that nearest-pair coherence rises from zero and inactive directions acquire nonzero readouts when two chosen directions overlap. That is the concrete cost of packing more features into the same space.</p>
    <SuperpositionExplorer tryThis={{ do: 'Start with 2 features, then set the count to 8 and choose several adjacent pairs.', notice: 'nearest-pair coherence rises from 0.00 to 0.92, and inactive directions receive nonzero readouts because all eight features cannot stay orthogonal in two dimensions.' }} />
    ...
    <p style={prose}>A 2025 move extends this work from isolated features to prompt-specific feature-level computational graphs. Ameisen et al. (2025) [7] replace MLPs with a learned cross-layer transcoder, a proxy whose features read from one residual stream and write to later MLP outputs, then trace attribution graphs from a target logit through feature-to-feature contributions. The result is a testable graph rather than a catalog of interesting features, but it is only partial coverage: reconstruction error, pruning and visualization choices, missing QK attention circuitry, and imperfect mechanistic faithfulness can all create tool artifacts or leave a critical computation unexplained.</p>
    ...
    <p style={prose}>For a worked toy example, take <InlineMath>{'h=(0.25,0.35)'}</InlineMath>, <InlineMath>{'v=(1,0.15)'}</InlineMath>, and <InlineMath>{'\\alpha=0.8'}</InlineMath>. Substitution gives the following state and, under the frozen decoder used below, its three logits and softmax probabilities.</p>
    <MathBlock>{'$$h^{\\prime}=(0.25,0.35)+0.8(1,0.15)=(1.05,0.47)\\\\[4pt]\\operatorname{softmax}(0.8455,1.3900,0.1260)=(31.15\\%,53.69\\%,15.17\\%)$$'}</MathBlock>
    <p style={prose}>The first line is the vector addition; the second applies softmax to the toy decoder's logits for direct, warm, and cautious answers.</p>
