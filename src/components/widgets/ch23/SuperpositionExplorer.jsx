...
export default function SuperpositionExplorer({ tryThis }) {
  ...
  return <WidgetCard title="Pack more feature directions into two dimensions" number="23.2" tryThis={tryThis}>
    <p style={{ margin: '0 0 16px', fontFamily: "'Inter', sans-serif", fontSize: 12, lineHeight: 1.55, color: 'var(--text-mid)' }}>
      This two-dimensional toy encodes two active features by adding their unit directions. Each feature is read out with a dot product. With more directions than dimensions, non-selected directions receive nonzero readout: that cross-talk is the displayed interference. Real superposition is learned, nonlinear, and often sparse; this is the geometric core.
    </p>
    <label style={{ display: 'block', fontFamily: mono, fontSize: 11, color: 'var(--text)', marginBottom: 12 }}>
      ...
      <input aria-label="Number of superposed features" type="range" min="2" max="9" step="1" value={count} onChange={e => { const next = Number(e.target.value); setCount(next); setActive(v => v % next); }} style={{ display: 'block', width: '100%', marginTop: 8 }} />
    </label>
    <label style={{ display: 'block', fontFamily: mono, fontSize: 11, color: 'var(--text)', marginBottom: 18 }}>
      first active feature = f{model.chosen + 1}; second = f{model.partner + 1} at 0.65
      <input aria-label="First active feature in superposition toy" type="range" min="0" max={count - 1} step="1" value={model.chosen} onChange={e => setActive(Number(e.target.value))} style={{ display: 'block', width: '100%', marginTop: 8 }} />
    </label>
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1fr) minmax(190px, .9fr)', gap: 18 }}>
      ...
      <div style={{ padding: 14, background: 'var(--code-bg)', borderRadius: 6 }}>
        ...
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border)', fontFamily: mono, fontSize: 11, color: 'var(--text)' }}>max pair coherence <span style={{ color: 'var(--accent)' }}>{model.coherence.toFixed(2)}</span><br />mean inactive cross-talk <span style={{ color: 'var(--purple)' }}>{model.interference.toFixed(2)}</span></div>
      </div>
    </div>
  </WidgetCard>;
}
